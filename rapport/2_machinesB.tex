...
\noindent
\bcode{ (reg1 $\in $ INT $\&$ -1024 <= reg1 $\&$ reg1 <= 1023) $\wedge$}\\
\bcode{ (xx $\in $ INT $\&$ -256 <= xx $\&$ xx <= 255)}\\
\bcode{ $\Rightarrow$ }\\
\bcode{ [  zz := xx + reg1;} \\
\bcode{ $~$ yy $\leftarrow$ bound(-1024, zz, 1023);}\\
\bcode{ $~$ reg1 := yy ]}\\
\bcode{ $\neg$ [ yy $\in $: { yy | yy $\in$ INT $\&$ -1024 <= yy $\&$ yy <=
1023} ]}\\
\bcode{ $\neg$ ( reg1 $\in$ INT $\&$ -1024 <= reg1 $\&$ reg1 <= 1023)}\\
