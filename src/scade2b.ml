...
let scade_file, xml_file =
  ...
  (match !dir with 
    Some n -> (Filename.current_dir_name n)^"/KCG/kcg_xml_filter_out.scade", 
      (Filename.current_dir_name n)^"/KCG/kcg_traces.xml"
...
(* Récupération d'une map de noeuds qui sont indexés par leur nom, ainsi qu'une map de constantes également indexées par leur nom. *)
  ...
  let lexbuf = Lexing.from_channel channel in
  try
    let (node_map, const_list) = Parser_prog.prog Lexer_prog.token lexbuf in
    close_in channel;
  with
