...
(* ast_repr to ast_norm_repr functions *)
let rec p_expr_to_n_expr = function
  ...
  | PE_App (id, elist) -> NE_App (id, (List.map p_expr_to_n_expr elist))
  | PE_Bop (bop, e1, e2) -> NE_Bop (bop, p_expr_to_n_expr e1, p_expr_to_n_expr e2)
  | PE_Unop (unop, e) -> NE_Unop (unop, p_expr_to_n_expr e)
  | PE_Fby (e1, e2) -> failwith "Arrow missed in normalizer"
  | PE_Pre e -> failwith "Pre missed in normalizer"
...
let handle_assert main_node asser = 
  ...
  let rec ident_finder = function
    | PE_Ident iden -> if (!id <> "" && !id <> iden) then raise (Assert_id_error !id) else id := iden
  ...
  and idarray_finder = function
    ...
    | PA_Slice (iden, l) -> if (!id <> "" && !id <> iden) then raise (Assert_id_error !id) else id := iden; 
      List.iter (fun (e1, e2) -> ident_finder e1; ident_finder e2) l
  in
  ident_finder asser;
...
let handle_register = 
  let cpt = ref 0 in
  fun main_node eq -> match eq with
  | P_Eq (l, PE_Fby (init, PE_Pre (PE_Ident id))) ->
    incr cpt;
    let typ = match find_type id (main_node.p_param_in@main_node.p_param_out@main_node.p_vars) with
      | Some typ -> typ
      | None -> raise Register_error
    in
    let new_id = "reg__" ^ (string_of_int !cpt) in
    let reg = { reg_id = new_id;
    		reg_type = p_type_to_n_type typ;
    		reg_ini = p_expr_to_n_expr init;
    		reg_var = NE_Ident id;
    	      } in
...
let ident_of_expr expr =
  ...
  let rec idexpr_rec = function
    ...
    | NE_App (id, elist) -> List.iter idexpr_rec elist
...
let folder main_node = 
  ...
  let normalize_eq res = function
    | P_Eq (lp, expr) as eq -> (
      match expr with
      | PE_Fby (_, _) -> let (reg, reg_eq) = handle_register main_node eq in
  			 registres := reg:: !registres; 
			 reg_eq::res
      | _ -> (plp_to_nlp lp, p_expr_to_n_expr expr)::res
    )
    | P_Assert expr ->
  ...
  let id_regs = List.fold_left (fun id_list reg -> reg.reg_id::id_list) [] !registres in
  let scheduled_eqs = 
    let (id_inputs, _) = List.split inputs in
    scheduler eqs (id_inputs@id_regs) in
